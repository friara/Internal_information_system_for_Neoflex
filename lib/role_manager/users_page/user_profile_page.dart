...
class _UserProfilePageState extends State<UserProfilePage> {
  ...
  Future<void> _uploadAvatar(File imageFile) async {
    try {
      ...
      final response = widget.isAdmin
          ? await GetIt.I<Openapi>().getUserControllerApi().adminUploadAvatar(
              id: int.parse(widget.userData['id'] ?? '0'),
              file: multipartFile,
              headers: {
                'Authorization':
  ...
  void _checkUserPermissions() {
    ...
    // Упрощенная проверка - пользователь может редактировать свой профиль
    _isCurrentUser = profileUserId != null &&
        currentUserId != null &&
        profileUserId == currentUserId;

    // Разрешаем изменение аватара только для своего профиля
    _canChangeAvatar = _isCurrentUser;

    setState(() {
      _isRoleLoaded = true;
    });

    debugPrint('Проверка прав доступа:');
    debugPrint('ID профиля: $profileUserId');
    debugPrint('ID текущего пользователя: $currentUserId');
    debugPrint('Это текущий пользователь: $_isCurrentUser');
    debugPrint('Может менять аватар: $_canChangeAvatar');
  }
  ...
  // Widget _buildAvatarContent(String avatarBaseUrl, String? avatarUrl) {
  //   if (_avatarImage != null) {
  //     return ClipOval(
  //       child: Image.file(
  //         _avatarImage!,
...
//           _showSnackBar('Пожалуйста, выберите изображение', isError: true);
//           return;
//         }
...
//       }
//     } catch (e) {
//       _showSnackBar('Ошибка при выборе изображения: ${e.toString()}',
//           isError: true);
//       debugPrint('Ошибка выбора изображения: ${e.toString()}');
//     }
//   }
...
//     return GestureDetector(
//       onTap: _isCurrentUser || widget.isAdmin ? _pickImage : null,
//       child: Stack(
//         children: [
//           CircleAvatar(
//             radius: 50,
//             backgroundColor: Colors.grey[200],
//             child: _buildAvatarContent(avatarBaseUrl, avatarUrl),
//           ),
//           if (_isCurrentUser || widget.isAdmin)
//             Positioned(
//               bottom: 0,
//               right: 0,
//               child: Container(
...
// //     final avatarBaseUrl = dio.options.baseUrl;
// //     final avatarUrl = _avatarUrl ?? widget.userData['avatarUrl'];
// //     final isCurrentUserProfile = _isCurrentUser;
