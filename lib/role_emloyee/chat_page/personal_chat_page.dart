...
class _PersonalChatPageState extends State<PersonalChatPage> {
    ...
    if (isImage) {
      ...
      try {
        ...
        if (result.type != ResultType.done) {
          _showErrorSnackbar('Не удалось открыть видео: ${result.message}');
        }
      } catch (e) {
        debugPrint('Ошибка открытия видео: $e');
        _showErrorSnackbar('Ошибка открытия видео: ${e.toString()}');
      }
    } else {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            backgroundColor: Colors.grey[800],
            content: Text('Файл сохранен: ${directory.path}'),
            duration: const Duration(seconds: 2),
          ),
        );
  ...
  Future<void> _downloadAndOpenFile(FileDTO fileDto) async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          backgroundColor: Colors.grey[800],
          content: Text('Открытие $fileName...'),
          duration: const Duration(seconds: 2),
        ),
      );
  ...
  Future<void> _openMediaInViewer(FileDTO file) async {
    ...
    if (isImage) {
      ...
      try {
        ...
        final fileName =
            file.fileName ?? 'video_${DateTime.now().millisecondsSinceEpoch}';
        final fileExtension = file.fileType?.split('/').last ?? 'mp4';
        final savePath = '${tempDir.path}/$fileName.$fileExtension';

        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            backgroundColor: Colors.grey[800],
            content: Text('Подготовка видео...'),
            duration: const Duration(seconds: 2),
          ),
        );
              ...
              children: [
                ...
                if (file.uploadProgress != null)
                  Positioned(
                    ...
                    child: Center(
                      child: Text(
                        '${(file.uploadProgress! * 100).toStringAsFixed(0)}%',
                        style: const TextStyle(
                          color: Colors.white,
                          fontSize: 12,
                          fontWeight: FontWeight.bold,
  ...
  void _showChatInfo() {
    if (_chat == null) {
      _showErrorSnackbar('Данные чата не загружены');
      return;
    }
...
//       );
//     } else if (isVideo) {
//       // Для видео сразу открываем через стороннее приложение
//       try {
//         final result = await OpenFile.open(file.path!);
//         if (result.type != ResultType.done) {
//           _showErrorSnackbar('Не удалось открыть видео: ${result.message}');
...
//   void _showPrivateChatParticipant() {
//     // Используем otherUserId из ChatDTO если он есть, иначе находим собеседника вручную
//     final otherUserId = _chat?.otherUserId ??
//         _chat?.participantIds
//             ?.firstWhereOrNull((id) => id != widget.currentUserId);
