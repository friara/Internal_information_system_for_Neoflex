...
class NewsFeedState extends State<NewsFeed> {
  ...
  void _checkForPostCreationMessage() {
    ...
    if (routeArgs?['postCreated'] == true &&
      ...
      setState(() {
        _postCreationMessageShown = true;
      });
  ...
  Future<void> _loadNotificationCount() async {
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Ошибка загрузки уведомлений')),
        );
      }
    }
  }
  ...
  Future<void> _loadUserRole() async {
    try {
      ...
      if (e.response?.statusCode == 401) {
        ...
        setState(() {
          _errorMessage = 'Ошибка загрузки роли пользователя';
        });
      }
    } catch (e) {
      if (mounted) {
  ...
  Future<void> loadPosts() async {
    ...
    try {
      ...
      final newPosts = await Future.wait(postDTOs.map((dto) async {
        ...
        //post.createdWhen = post.createdWhen.toLocal();
            ...
            post.comments =
              ...
              if (c.userId == null) {
                return Comment(
                  ...
                  userName: 'Аноним',
                  createdAt:
                      c.createdWhen?.toLocal() ?? DateTime.now().toLocal(),
                );
              }
      ...
      if (e.response?.statusCode == 401) {
        ...
        setState(() {
          _errorMessage = 'Ошибка загрузки постов';
        });
      }
    } catch (e) {
      if (mounted) {
        ...
        setState(() {
          filteredPosts[index].comments.add(Comment(
                ...
                userName: user != null
                    ? '${user.firstName ?? ''} ${user.lastName ?? ''}'.trim()
                    : 'Аноним',
                createdAt: response.data!.createdWhen?.toLocal() ??
  ...
  Future<void> toggleLike(int index) async {
    ...
    final post = filteredPosts[index];
    if (post.id == null || _currentUserId == null) return;
    ...
    try {
      if (post.isLiked) {
        await _likeApi.deleteLike(postId: post.id!);
        setState(() {
          filteredPosts[index].isLiked = false;
          filteredPosts[index].likesCount--;
        });
      } else {
        await _likeApi.createLike(postId: post.id!);
        setState(() {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Произошла ошибка при изменении лайка')),
        );
      }
    } finally {
      if (mounted) {
                              ...
                              return GestureDetector(
                                ...
                                child: Container(
                                  ...
                                  child: ListTile(
                                    ...
                                    title: Text(
                                      comment.userName,
                                      style: AppStyles.textForCommentForAvtor,
                                    ),
                                    subtitle: Column(
                                      crossAxisAlignment:
                                      ...
                                      children: [
                                        Text(
                                          comment.text,
                                          style: AppStyles.textForComment,
                                        ),
